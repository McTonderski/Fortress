...
def display_button(id, text):
    text_length = display.measure_text(text, ITEM_TEXT_SIZE)
    pos = 0
    if id == 0:
        pos = int(0 + 20)
    elif id == 1:
        pos = int(WIDTH // 3 - (text_length // 2)) + 50
    else:
        pos = int(WIDTH - text_length - 15)
    # pos_int = int(pos)
    display.text(text, pos, int(HEIGHT - ITEM_TEXT_SIZE) - 5, WIDTH, ITEM_TEXT_SIZE)
...
def draw_page():
    ...
    display.set_pen(15)
    display.clear()
    if len(container_names) > 0 and not button_pressed:
        display.set_font("bitmap6")
        for id, container in enumerate(
            container_names[
                ITEMS_PER_PAGE
                * (selected // ITEMS_PER_PAGE) : ITEMS_PER_PAGE
                * (selected // ITEMS_PER_PAGE)
                + ITEMS_PER_PAGE
            ]
        ):
            display.set_pen(0)
            if id == selected % ITEMS_PER_PAGE:
                display.rectangle(0, 4 + ((id % ITEMS_PER_PAGE) * 10), WIDTH, 10)
                display.set_pen(15)
            header = f"{id + (ITEMS_PER_PAGE * (selected // ITEMS_PER_PAGE)) + 1}: {container.name}"
        ...
        display_button(0, "toggle")
        display_button(1, "restart")
        if container_names[selected].repo_source:
            display_button(2, "rebuild")
        ...
        if _err == "":
            display.text(
                f"Container {container_names[selected].name} is {button_pressed}",
                5,
                65,
                WIDTH,
                1,
